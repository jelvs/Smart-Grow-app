...
class HumidityGraph : AppCompatActivity() {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_humidity_graph)

        val goBack = findViewById<ImageButton>(R.id.gobackhumidity)
    ...
    private fun buildChart(numberOfReadings: Int) {
        HttpTask {
            if (it == null) {
                Toast.makeText(this, "Error checking current humidity", Toast.LENGTH_LONG).show()
            } else {
                val response = JSONArray(it)
                setHumidityData(response, numberOfReadings)
            }
        }.execute("GET", "https://api.smartgrow.space/humidity?readings=$numberOfReadings")
    }

    @SuppressLint("NewApi")
    private fun setHumidityData(data: JSONArray, numberOfReadings: Int) {
        ...
        val set1 = LineDataSet(values, "Humidity")
        set1.lineWidth = 5f
        ...
        val limitLineUpper = LimitLine(60f, "Too Moist")
